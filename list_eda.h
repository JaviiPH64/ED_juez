...
protected:
	...
	class Iterador {
		...
		Nodo* act;
		// puntero al nodo fantasma (para sabr cuándo estamos fuera)
		Nodo* fan;
...
public:
	...
	*/
	...
	// devuelven un iterador constante al final del recorrido y fuera de éste
	...
	*/
	...
	// devuelve un iterador al final del recorrido y fuera de éste
	iterator end() {
	...
	iterator insert(iterator const& it, T const& elem) {
		...
		Nodo* nuevo = this->inseta_elem(elem, it.act->ant, it.act);
	...
	iterator eras(iterator const& it) {
		assert(this->fantasma == it.fan); // comprueba que el iterador es de esta lista
