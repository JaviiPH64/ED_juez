...
protected:
	...
	y dos punteros, uno al nodo anterior y el otro al nodo siguiente
	*/
	struct Nodo {
...
public:
	...
	T const& front() const {
		if (empty()) {
			throw std::domain_error("la dcola vacía no tiene primero");
		}
	...
	T const& back() const {
		if (empty()) {
			throw std::domain_error("la dcola vacía no tiene último");
		}
	...
	void pop_front() {
		if (empty()) {
			throw std::domain_error("eliminando el primero de una dcola vacía");
		}
	...
	void pop_back() {
		if (empty()) {
			throw std::domain_error("eliminando el último una dcola vacía");
		}
...
protected:
	...
	Nodo* inseta_elem(T const& e, Nodo* anterior, Nodo* siguiente) {
		anterior->sig = nuevo;
		siguiente->ant = nuevo;
		++nelemes;
		return nuevo;
